// Papendal video-eindkaart 1:1 — scene voor animations-v2 engine
// Vierkante variant van eindkaart-scene-standalone.jsx. Zelfde tekst,
// animatie en timing; alleen de verticale opbouw is hertekend voor 1080x1080.
// Het canvas is even breed als de 9:16-versie, dus de korpsgrootte (92) blijft
// gelijk en de regels breken identiek.
(() => {
  const ORANGE = '#F1740E';
  const FONT = "'Anton', sans-serif";

  // Verticale opbouw van het vierkante canvas. Het middenvlak tussen de twee
  // tekstblokken blijft leeg, zodat de kaart ook over beeld met een persoon
  // in het midden gelegd kan worden.
  const CANVAS = 1080;
  const TITLE_TOP = 150;     // bovenkant blok 1 (JOUW / THUISBASIS)
  const SUBTITLE_TOP = 555;  // bovenkant blok 2 (VOOR / TOPPRESTATIES)
  const BADGE_BOTTOM = 140;  // afstand van de badge tot de onderrand
  ...
  function Badge() {
    ...
    return (
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: BADGE_BOTTOM, display: 'flex', justifyContent: 'center' }}>
  ...
  function EndCard() {
    return (
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
        <XMotif />
        <div style={{ position: 'absolute', left: 0, right: 0, top: TITLE_TOP, display: 'grid', justifyItems: 'center' }}>
        ...
        <div style={{ position: 'absolute', left: 0, right: 0, top: SUBTITLE_TOP, display: 'grid', justifyItems: 'center' }}>
  ...
  function App() {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1a1a1a' }}>
        <SceneStage width={CANVAS} height={CANVAS} bg="transparent"
  ...
  window.PapendalEndCardSquare = App;
})();
